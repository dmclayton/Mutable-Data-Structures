...
algorithm~$\Qry$ may err in its response to queries.  Roughly
...
theirs in several ways, as we will detail, shortly.  One important
extension is the consideration of data structure privacy.
...
\heading{Data structure privacy.}
%
...
%
%\anytodo{Fix citations below.}
%
But one can hope to leak \emph{nothing more} than what is absolutely
necessary, and
%\footnote{If more than $|\pub|$ is necessary to leak, the ``trivial'' privacy
%solution of encrypting $\pub$ is not applicable.}
this goal leads to our first notion of privacy.
%In the tradition of the original semantic-security notion for
%encryption~\cite{goldwasser1982ss}, and like more recent notions of privacy
%for property-preserving encryption~\cite{xxx} and searchable
%encryption~\cite{xxx},
...
\if{0}{
  \anytodo{Motivate applications of secret-key data structures.  A consistent
  reviewer complaint was the separation between $\pub$ and $\privaux$ as being
  artificial and not motivated.  We need to address this vigorously here.  For
  example, in a CDN, Bloom filters are passed between content servers; likewise
  p2p networks, where trust may not be assumed.  Using something like Squid, even
  if you don't transmit your bit array, the hash functions are public because
  they are specified in the publicly available code.  } \jnote{Need to think more
  about this. I don't see a setting where (1)~privacy is desired but
  (2)~encryption cannot be used to hide $\pub$ from an attacker.}
}\fi
...
%
\if{0}{
  Even within this class of data structures, we are only able to
  thoroughly consider two types, set-membership data structures and dictionaries.
  Our formalisms support future work to analyze other immutable structures.
}\fi
%
