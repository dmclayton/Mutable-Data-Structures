...
\heading{False-positive probbility in the non-adaptive setting}
Let~$\rho$ be a function, $\lambda\geq0$ be an intger, and define
$\Pi = \bloom[\id^\rho,n,\lambda] = (\Rep^\rho, \Qry^\rho, \Up^\rho)$ as in
Figure~\ref{fig:bf-def}.
%
...
\end{equation}
%
That is, $P_\bff(k,m,n)$ is the probability that any~$x$ a false positive for
the representation of some~$\setS$ when a random function is used for hashing.
%
...
particular choices of $k$ and $m$ as functions of~$n$
\[
  P_\bff(k,m,n) = \lim_{n\goesto\infty} (1-e^{-kn/m})^k \,.
\]
%
In the same paper, they demonstrate via simulation that this is a very good
approximation of the false positive probability for parameters of practical
interest.

\heading{Error function for set-mempership queries}
%
Throughout this section we will use the error function $\delta_\bff$ as
\begin{equation}
  \delta_\bff(x, y) =
  \begin{cases}
    0 & \text{if}\ x=y \\
