...
\todo{DC}{Revise this. It should be a summary of all of the results: basic BFs
are week; salted BFs are fine, but we need bigger ones in the public setting and
we can't allow mutations (in the private setting we allow mutations and get
better bounds for smaller filters; keyed BFs perform well in the public, mutable
setting; thresholding yields better bounds than the usual capped appraoch.}
%
These results show that the standard Bloom filter construction is not secure
against an adaptive adversary. The best defensive strategy against potential
attacks depends on the setting. If the representation is never updated, then
using a per-representation salt and a sufficiently large filter is enough.
