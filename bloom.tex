...
\begin{theorem}[\errep1 security of salted BFs]\label{thm:sbf-errep1}
  ...
  \begin{equation*}
    \begin{aligned}
      ...
      \hspace{20pt}
        q_R \cdot \left(\frac{q_H}{2^\lambda} + \left(\frac{e^{\alpha-1}}{\alpha^\alpha}\right)^{pq}\right) \,,
    \end{aligned}
  \end{equation*}
  where $H$ is modeled as a random oracle, $q = q_H + q_T$, $p$ is the standard (non-adaptive) false positive probability for a Bloom filter with the chosen parameters, and $\alpha = \frac{r}{pq}$.
  %
...
\todo{DC (lead)}{Add interpretation of the bound.
\begin{enumerate}
  \item Recall the Kirsch-Mitzenmacher bound~\cite{kirsch2006less}.
    I.e., explain this thing: $(1 - e^{-kn/m})^k + O(1/m)$.
  \item Plot the bound for ``reasonable'' values of $p$, $q_H$, $q_T$, and~$r$.
  \item Figure out how to write the bound down.
\end{enumerate}
}
...
\begin{theorem}[\erreps1 security of salted BFs]\label{thm:sbf-erreps}
...
Theorem~\ref{thm:sbf-errep1}. In an earlier iteration this was the first result
...
$\game_0$ to $\game_1$ transition in Theorem~\ref{thm:sbf-errep1}. I think it's
