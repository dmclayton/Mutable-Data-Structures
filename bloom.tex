\begin{figure}
  ...
  \caption{Top: Keyed structure $\bloom[R,n,\lambda]$ given by
  ...
  %
  Bottom: A slightly modified structure, $\bloom_\mathrm{ft}[R,\ell,\lambda]$ given by
...
\todo{DC}{Revise this text (now out-of-date!). ``In this section we consider two
classes of the Bloom filters, each employing a different thresholding strategy.
[We will have discussed why different strategies are needed in the intro, at
least at a high level.] Let $R:\keys\by\bits^*\to[m]^k$ be a function and let
$n,\ell,\lambda\geq0$ be integers. The first class, defined at the top of
Figure~\ref{fig:bf-def}, ...'' Don't define the concrete structures, as we did
before. Instead, define the concrete structure at the top of the section in
which it appears.}
%
...
%
Deriving a tight, concrete uppper bound is illusive; \todo{DC}{Briefly comment
...
example, some web spiders \cpnote{``Web spiders''? Do you mean web crawlers?  In
either case, ensure that the reader knows what this means.} use Bloom filters to
store the set of webpages which have already been visited during a crawl.
Suppose the adversary controls at least one such webpage and wishes to deny the
...
%
\cpnote{Overall, I think this part needs to be revised to make it more clear.
...
particular, depending on the choice of hash, the adversary can compute the hash
function on its own in order to determine which choices will set the maximum
number of bits to 1, or which choices will set certain target bits to 1 in order
to cause specific false positives. They show that with some reasonable parameter
choices \cpnote{and a particular ``bad'' choice of hash function? what are the
parameters?}, the
adversary can double the false positive rate if they control a third of the
inputs to the filter.

Gerbet \etal discuss multiple ways to mitigate pollution attacks, including
\st{using alternate data structures besides Bloom filters}\cpnote{not worth
mentioning this here} or choosing the parameters
of the filter pesimistically, so that even if a pollution attack occurs, the
false positive rate is kept below some threshold of acceptability. \st{Both} of these
...
practice.
%
\cpnote{Well, we're suggesting 3KB for representing just $n$ elements! How does
our size explosion compare to theirs?}
%
...
\todo{DC}{Simplify the description of this attack, as well as the next
...
%
\todo{DC}{Be sure the reader knows that class of BF you're talking about, i.e.,
how we're thresholding. (At this point I have no idea.)}
...
For integers $q_R, q_T, q_H, r, t \geq 0$, if $r > p'q_T$, it holds that
  \begin{equation*}
    \begin{aligned}
