...
\begin{figure}
  ...
  }
  {
    ...
    \underline{$\Up^R(\langle M, \salt \rangle,x)$}\\[2pt]
      ...
      return $\langle M \vee \bmap_m(R(x)), Z \rangle$
  }
  \caption{A keyless structure $\bloom[R,\ell,n,\lambda] = (\Rep^R,\Qry^R,\Up^R)$
  used to define Bloom filter variants. It ``compiles'' a function $R: \bits^*
  \to [m]^k$ (either a hash function or keyed PRF), initial threshold $n\geq0$,
  update threshold $\ell\geq0$, and salt length $\lambda\geq0$ into a concrete
  scheme. Functions~$\hw$ and~$\bmap_m$ are defined in
...
The formal Bloom filter construction is given in Figure~\ref{fig:bf-def}. From this
...
integers.
%
The standard Bloom filter is the structure $\BF[H,\ell,n] = \bloom[H,\ell,n,0]$, which we will
term the \emph{basic} Bloom filter. It has no key and does not employ a salt.
%
The \emph{salted} Bloom filter $\SBF[H,\ell,n,\lambda] = \bloom[H,\ell,n,\lambda]$ is the same except that
...
filter $\KBF[F,\ell,n,\lambda]$ is defined as follows. On input of $(K,\setS)$, the
...
Fix $H:\bits^*\to[m]^k$ and integers $\ell,n\geq0$ and let $\BF[H,\ell,n] = (\Rep^H, \Qry^H,
\Up^H)$ as defined above.
...
This \erreps1 attack succeeds with probability~$1$ assuming a covering set can be found.
...
%
Fix function $H:\bits^*\to[m]^k$ and integers $\ell,n,\lambda\geq0$ and let
...
following. Let $\delta:\bits^2\to\N$ be the error function defined by
$\delta(a,b) = |a-b|$.
%
