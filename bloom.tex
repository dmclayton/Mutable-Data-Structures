...
\begin{figure}
  \oneCol{0.46}
  {
    ...
    \underline{$\Up^R(\langle M, \salt \rangle,x)$}\\[2pt]
      $U \gets M \vee \bmap_m(R(x))$;
      if $\hw(U) > t$ then return $\bot$\\
      return $U$
  }
  \caption{A keyless structure $\bloom[R,t,\lambda] = (\Rep^R,\Qry^R,\Up^R)$
...
integers.
%
The standard Bloom filter is the structure $\BF[H,t] = \bloom[H,t,0]$, which we will
term the \emph{basic} Bloom filter. It has no key and does not employ a salt.
%
The \emph{salted} Bloom filter $\SBF[H,t,\lambda] = \bloom[H,t,\lambda]$ is the same except that
...
filter $\KBF[F,t,\lambda]$ is defined as follows. On input of $(K,\setS)$, the
...
%
\cpnote{This attack only applies when $t=\infty$; I don't know how effective it
would be if $t$ were fixed.}
%
