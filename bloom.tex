...
\begin{theorem}[\errep1 security of salted BFs]\label{thm:sbf-errep1}
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  where $H$ is modeled as a random oracle, $q = q_H + q_T$, $p$ is the standard
  (non-adaptive) false positive probability for a Bloom filter with the chosen
  parameters, and $\alpha = \frac{r}{pq}$.
  %
