...
Once we have reduced the adversary to randomly guessing queries, an attack can
be modeled using the binomial distribution. Each guess has a fixed chance $p$ of
producing a false positive, and the adversary succeeds if it accumulates at
least $r$ successes over the course of $q$ attempts. The bound given in the
theorem statement can then be found using a standard Chernoff bound on Poisson
distributions, namely
\[
  \Prob{X \ge (1+\delta)\mu} < \left(\frac{e^\delta}{(1+\delta)^{1+\delta}}\right)^\mu
\]

\todo{DC}{Add interpretation of the bound. It will look like nonsense to most
readers, so help them out. For what parameters is it comparable to the
non-adaptive false-positive probability? We'll want to plot this for reasonable
values of $k, m, n, q_H, q_T, r$.}

The bound given in the theorem statement is not directly comparable to the
standard bounds for a Bloom filter, which are typically given in terms of the
false positive probability per query rather than the probability of accumulating
some target number of false positives over a larger number of queries.
