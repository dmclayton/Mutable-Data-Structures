\begin{figure}
  \twoColsNoDivide{0.22}
  {
    \underline{$\Rep^R_K(\col)$}\hfill\diffminus{$\sketch$}\diffplus{$\sketch_\mathrm{ft}$}\\[2pt]
      $\salt \getsr \bits^\lambda$\\
      for $i$ in $[1..k]$ do\\
        $\tab M[i] \gets 0^m$\\
      $\pub \gets \langle M, \salt$\diffminus{$, 0$}$\rangle$
      for $x \in \col$ do \\
  ...
  {
    \underline{$\Qry^R_K(\langle M, \salt$\diffminus{$, c$}$\rangle,\qry_x)$}\\[2pt]
      ...
      return $a$
    \\[6pt]
    \underline{$\Up^R_K(\langle M, \salt$\diffminus{$, c$}$\rangle,\up_{x,b})$}\\[2pt]
      \diffminus{if $c \geq n$ then return $\bot$}\\
      \diffplus{if $\hw'(M) > \ell$ then return $\bot$}\\
      $M' \gets M$;
      $X \gets R_K(\salt \cat x)$\\
      for $i$ in $[1..k]$ do\\
  ...
  $\ell$-thresholded construction $\sketch_\mathrm{ft}[R,\ell,\lambda]$, which
...
As in the case of Bloom filters, it is significantly easier to work with an
