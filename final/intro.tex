...
Bloom-filter-like structures. A Bloom filter acts as a representation
of a set~$\col$. Instead of storing exact containment information, the filter is
a length-$m$ bit-array (initially all zeros), where~$m$ is much less than the
number of bits needed to store~$\col$ in full.  Elements $x \in \col$ are added
to the representation by computing multiple hash values
$h_1(x),h_2(x),\ldots,h_k(x)\in [m]$, then setting the indicated array positions
to~$1$.  A Bloom filter supports set membership queries, i.e., ``is
$x\in\col$?'', by hashing~$x$ and responding positively iff all of the indicated
positions hold a 1-bit. We will often refer to Bloom filters and similar
structures as `representations' because they represent the data from a set or
multiset without actually storing the entire (multi)set in full. Because of the
compact nature of a Bloom filter, query-response errors are admitted.
...
When admitted queries may return non-binary results, e.g.,
frequency-of-element queries, defining a notion of `error' becomes non-obvious.
Even in the traditional, non-adaptive setting, one is
