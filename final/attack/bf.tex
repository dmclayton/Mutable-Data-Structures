...
%
The possibility of pre-computing the structure in the \erreps1 experiment yields the
...
with probability~$1$.
%
Next we describe a heuristic strategy for finding a covering set and evaluate
...
%
In particular, there is a simple heuristic strategy for finding a
...
of sets checked for a number of simulations and various parameters. (For each
simulation we start with a ``fresh'' choice of random oracle.)
%
...
choices, the basic Bloom filter is not secure in the \erreps1\ sense.
%
Finally, we find that the average number of sets that were tested is about
$O(kr)$ within all of the parameter regimes we studied.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.7]{fig/bf-correct-attk-sim}
  \caption{
    %
  Success rate and average-case time complexity for the optimized attack on classic
  Bloom filter.
  %
  Each plot shows the success rate ({\scriptsize\ding{108}}), as well as the average number
  of tested sets ({\scriptsize\ding{53}}), for 1000 executions of
