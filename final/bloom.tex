...
PRF.
%
Note that the basic and salted BFs have key space $\{\emptystr\}$ and the keyed
...
\emph{$\ell$-thresholded} filters. Instead of rejecting updates after a pre-determined
...
determine which choices maximize the number of bits set to 1, or which
...
the~$q$ term overly conservative. In these settings, a significantly smaller
...
\begin{figure}
  ...
  \caption{
    The value of $\zeta_{k,m,n}(q,r)$ (Equation~(\ref{eq:zeta})) for: $k=16$;
    $q=2^{64}$ (top) or $2^{32}$ (bottom); $n=100$ (left) or~$10^9$ (right);
    varying values of~$r$ (one line per $r$-value); and varying values of the
    filter length~$m$ (the x-axis).  Note the log-2 scale on the y-axis.
  }
  \label{fig:bf-bound}
\end{figure}
...
\heading{Security in the mutable setting}
...
alternatively, filter size) can be further improved, especially if the number of
...
\begin{figure}
  ...
  \caption{
    ...
    $k=16$; $q=2^{32}$; $n=100$ (left) or~$10^9$ (right); $r=1$ (top) or~$5$
    (bottom); and varying $m$ (on the x-axis).
    %
