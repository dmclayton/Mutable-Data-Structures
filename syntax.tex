...
For an array~$M$ of integers, we analagously define $\hw'(X)$ to be the number
of \emph{nonzero} integers in the array. We also define $\zeroes(m)$ to be a
function taking a nonnegative integer and returning a length-$m$ integer vector
...
\todo{Any (Tom?)}{Fix the weird spacing in the last column of
