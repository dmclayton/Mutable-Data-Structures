...
%
For sets $\setX$ and $\setY$ and a keyspace $\keys$, we define a pseudorandom
function to be a function $F: \keys \times \setX \to \setY$. The intent is for
...
\begin{figure}
  ...
  }
  {
    \oraclev{$\PRFO(x)$}\\[2pt]
      ...
      if $T[x]$ is defined then return $T[x]$\\
      $T[x] \getsr Y$; return $T[x]$
  }
  \caption{The notion of PRF correctness, capturing indistinguishability from a
  random function.
  }
  \label{fig:prf-def}
\end{figure}
