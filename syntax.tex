...
Let $\Func(\setX,\setY)$ denote the set of functions $f:\setX\to\setY$. If
$\setY$, then we associate to this set the following distribution. For all
$x\in\setX$ and $y\in\setY$ it holds that
$\Prob{f\getsr\Func(\setX,\setY):f(x)=y} = |\setY|^{-1}$.
%
\todo{TS}{Decide if you're happy with this}

For every function~$f$, define $\id^f$ so that
$\id^f(\emptystr, x) = f(x)$ for all $x$ in the domain of~$f$.

\todo{Any (Tom?)}{Define function $\ticks(A,\cdot)$ for algorithm~$A$. Do we need big-O
notation in bounds?}
\dcnote{Not sure what this is supposed to mean.}
\cpnote{IIRC it's a relic of the old paper, one of Jon's contributions. I also
don't remember what this means, but I can dig it up if need be. It needs to be
defined or we need to drop it from the \errep1 to \errep\ lemma.}
...
updates, but some do, e.g. the Cuckoo filter~\cite{fan2014cuckoo}\todo{DC}{Fix
ref} and the stable
...
\todo{DC}{Decide if we still need this definition. If we don't use it
anywhere, then we don't need it.}
%
