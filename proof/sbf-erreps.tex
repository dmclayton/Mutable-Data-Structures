...
\end{eqnarray}
%
\todo{DC}{Don't assume the adversary never guesses the salt; argue that it's
irrelevant if it does or not.}
In~$\game_1$, we may now assume that the adversary never guesses the salt in its
$\HASHO_1$ queries. This means that none of the inputs to $\HASHO_1$ is ever
equal to any input to $\HASHO_2$ or $\HASHO_3$, both of which always use the
salt $\salt^*$. Since each $\HASHO$ is modeled as a random oracle, the outputs
of $\HASHO_1$ are therefore independent of the outputs of $\HASHO_2$ and
$\HASHO_3$.
%
...
%
\todo{DC}{Minimize the set of assumptions in light of this reduction. (I don't
...
to $\advC$.
%
\todo{DC}{Re-check the bound. From what I can tell, this step means you you need
to write $q_T+q_U$ in the final bound instead of $q_T$. MAKE SURE TO UPDATE THE
THEOREM, AS WELL AS THE EQUATIONS IN THE PROOF! I'm not going to look at this
proof again before submission.}
...
necessarily have $|\setX| \le q_T$\todo{DC}{Double check this isn't $q_T+q_U$}, and each $\qry_x \in \setX$ has some
