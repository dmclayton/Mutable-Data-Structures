...
This shows that, unless the adversary is able to guess the salt, access to $\HASHO_1$ is not beneficial for the adversary, and that the adversary cannot predict the behavior of $\QRYO$ or $\UPO$ in advance.

%

Furthermore, since each $\HASHO$ uses random sampling, each $\HASHO_2$ and $\HASHO_3$ call used for $\UPO$ and $\QRYO$ are independent of all other $\HASHO$ calls except in the case of repeated calls on the same element $x$. These repeated calls fall into one of three types:

\begin{enumerate}
  \item Calling $\QRYO(\qry_x)$ when $\up_x$ has already been sent to $\UPO$.
  \item Calling $\QRYO(\qry_x)$ when $\up_x$ has not already been sent to $\UPO$, but when $\qry_x$ has already been sent to $\QRYO$.
  \item Calling 
\end{enumerate}

From these arguments, we can assume without loss of generality that no $x$ is ever sent to $\HASHO_c$ twice for $c \in \{2,3\}$. Since the $\HASHO$ oracles use random sampling, all such calls are independent of each other. Since $\QRYO$ and $\UPO$ use $\HASHO$ calls to compute queries and updates, this means that all query and update operations are independent as well.

Since $\UPO$ calls are independent of all other oracle calls and the representation is private, the only influence $\UPO$ has on the game is to potentially increase the false positive probability by increasing the number of 1s in the filter. However, the number of 1s in the filter cannot increase beyond $\ell+k$ by the thresholding constraints on $\REPO$ and $\UPO$.

% no more than \ell+k bits will be set to 1, UNLESS more than that were set initially
% more than \ell bits will be set to 1
% if the adversary starts with a maximal set, 

However, we cannot quite reduce this game to the non-adaptive bionmial distribution, since it is possible for the adversary to update the filter based on responses to previous queries. We want to move to a game where it is not beneficial for the adversary to interleave updates and queries.
...
\end{enumerate}
Updates of type 1 are not helpful to the adversary, since doing so does not change the representation and actually decreases an entry in $\err$. Updates of the second and third type are identical except that 
