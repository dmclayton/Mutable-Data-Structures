\begin{figure*}
\todo{DC}{Fix the transciprtional errors here. (Remeber that we replaced
``$0^m$'' with $\zeroes(m)$. Take your time.}
\twoCols{0.47}
{
  \vspace{-7pt}
...
\todo{DC}{Re-check this argument from this point to the QED. There are some obvious errors, and it feels
like you moved fast through it. Take your time, and make sure you understand
it. (Try reading it out loud.)}
Analagously to the proof of Thereorem~\ref{thm:sbf-erreps-th}, we now move to a
...
each row of the sketch is a uniformly\todo{DC}{It's not uniformly random.} random bitmap \todo{DC}{Fix this. It's not
a bitmap} with $\ell+1$ out of $m$
