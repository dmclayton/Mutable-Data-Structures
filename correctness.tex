...
\heading{Keyless structures.}\tsnote{I've tried to organize these results into little sectionettes.  I don't know, yet, what is the right way to organize these generic results, but this is a start.  As you develop results, please try to keep them organized, whatever the organizational schema.} \dctodo{needs intro text.}
...
\begin{lemma}[\errep1 implies \errep for keyless structures]\label{lemma:errep}
  \dctodo{\errep1 undefined.} Let $\struct = (\Rep, \Qry, \Up)$ be a data structure with key
...
There is a similar equivalence between $\erreps1$ and $\erreps$; the reasoning is identical to that in the above proof. \dctodo{Needs a formal statement.  Perhaps just fold it into the previous lemma.}

\begin{lemma}[Salts do not affect \errep for keyless structures with public representation]\label{lemma:nosalt}
