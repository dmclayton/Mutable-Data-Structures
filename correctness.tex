...
% an adversary may want to find representations with large overlap, to search for false positives, or may want to find representations with small overlap, to find new elements to add to the set
% no other additions are necessary, so we only care about these extremes

%if you know the overlap of all representations created so far, is that equivalent to knowing the representations (up to permutation)?
% A \cap B has overlap 2
% A \cap C has overlap 2
% B \cap C has overlap 2
% A \cap B \cap C has overlap 1
% no, but only because you don't know exactly which elements are causing the overlap
% suppose the adversary begins by constructing only singleton sets
% this allows them to automatically detect false positives, and even determine exactly which elements will become false negatives if the false positive is `removed'
% this is not much different from the case of a standard Bloom filter, except that up to $k$ errors are produced with each false positive found... and once again there are issues when you run into the maximum allowed size of the filter; the adversary can choose a set with little overlap to produce a large number of 1s...
% can we prove that the adversary's best option is to only construct singleton sets?
% well, the adversary really only gets a single representation with updates
% successive updates differ only by 1
% this is equivalent to making singletons and calculating the differences between successive updates

We modify the $\UPO$ oracle so that it returns complete information about the overlap between all elements which have been previously handled by any oracle: any element of the original $\col$, any element inserted or removed at a later point, and any element which has been queried. Any information that could be gained through the $\QRYO$ oracle is now given automatically each time the representation is modified.

Because new $\QRYO$ and $\UPO$ calls are based on random sampling, this does not provide the adversary with any additional information about how additional elements which might be queried, inserted, or deleted in the future might behave. However, any element which is sent to $\QRYO$ or $\UPO$ which acts as a false positive will be immediately recognized as such.

We then modify the game so that producing a false positive gives the adversary credit to $\err$ equal to $\max(k \cdot d(0,1), d(1,0))$. Because false negatives can only be produced by deleting false positives, and the deletion of $n$ false positives can only produce as many as $nk$ false negatives, this means that the adversary has no need to cause false negatives. Doing so can only decrease the chances of producing additional errors by reducing the values of counters in the filter. The only additional concern is that the maximum capacity of the filter might be reached. [...]

Until the maximum capacity of the filter is reached, knowledge of the overlap between representation elements provides no additional useful information. However, after that point the adversary can optimize the filter by choosing a representation for a maximum-size set which has as little overlap between elements as possible. This will minimize the number of counters which are set to 0 and increase the expected number of false positives beyond the average for a maximum-capacity filter.

% What if we assume that the maximum capacity is not determined by the number of elements in the set, but instead by the sum over all counters?
% This also means the number of elements in $\col$ need not be kept track of
% Now the addition of false positives to the set does not bring the filter any closer to being full, so we do not need to worry about the adversary prematurely filling the filter when we have false positives be added to the set
% Furthermore, the adversary cannot "optimize" the filter except in making sure that the filter is in fact completely full, with exactly the maximum number of bits set to 1
% Can we turn the Bloom filter bound based on the number of elements contained in the filter into a bound based on the number of bits set to 1 in the filter?
% if b bits out of m are set to 1, the probability of a new element matching each of these is just (b/m)^k
...
Cuckoo filters provide a way of filtering elements with lower space overhead than counting filters while still allowing for deletion.
...
\begin{figure}
  ...
  }
  {
    ...
    \oraclev{$\UPO(\up_x)$}\\[2pt]
      $\setC \gets \emptyset$\\
      $a \gets \Qry[\HASHO_2](\pub, \qry_x)$\\
      if $a \neq [x \in \col]$ and $x \in \setC$ then\\
      \tab $\err \gets \err-1$\\
      if $b = 1$ then\\
      \tab $\col \gets \col \cup \{x\}$\\
      else\\
      \tab $\col \gets \col \setminus \{x\}$\\
      $\pub \gets \Up[\HASHO_2](\pub,\up_{x,b})$\\
      return~$\bot$
    \\[6pt]
    \oraclev{$\HASHO_2(\salt,x)$}\\[2pt]
      $\hh \getsr [m]^2$; $\vv \gets \fff(\hh$)\\
  ...
  }
  {
    \underline{$\G_3(\advB)$}\\[2pt]
    \oraclev{$\QRYO(x)$}\\[2pt]
      $a \gets \Qry[\HASHO_2](\pub, \qry_{x,1})$\\
      if $a \neq [x \in \col]$ then $\err \gets \err + 1$\\
      $\col \gets \col \union \{x\}$\\
      $\pub \gets \Up[\HASHO_2](\pub,\up_{x,1})$\\
      return~$a$
  }
  {
    \oraclev{$\UPO(\up_x)$}\\[2pt]
...
As with the case of a Bloom filter, the goal is to move to a game where the salted hash function is replaced by a true random function. Doing so requires that the adversary never be able to guess the hash function. In $\G_1$ we split the hash oracle into two oracles, giving the adversary direct access to $\HASHO_1$ while the other oracles make use of $\HASHO_2$. For any $\advA$ for $\G_0$, there is $\advB$ for $\G_1$ which produces the same advantage by simulating $\advA$. [...]
