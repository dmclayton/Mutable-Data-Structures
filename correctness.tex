...
%
Now, the array~$\err_i$ is indexed by query functions~$\qry$, and the value of $\err_i[\qry]$ is the ``cost'' of the error caused by~$\qry$, with respect to the \emph{current} data object~$\col_i$ and \emph{current} representation~$\pub_i$ (of~$\col_i$).  
%
...
When~$\QRYO(i,\qry)$ executes, the value in $\err_i[\qry]$ is overwritten iff the error caused by~$\qry$ is larger than the existing value of $\err_i[\qry]$.  The first time $(i,\qry)$ is queried to~$\QRYO$ this is guaranteed, since the minimum possible value of $d$ is 0.  
\ignore{
Since the set $\setC_i$ is used to prevent (WLOG) the adversary from repeating a query~$\QRYO(i,\qry)$ for a given~$\col_i$, 
}
Note that increases to the value of $\err_i[\qry]$ may only be made ``across'' updates to~$\col_i$.  This may seem to be overly conservative, as an error-heavy~$\col_i$ may become less so after an update.  But we account for this within the $\UPO$-oracle.
In particular, calls to the $\UPO$-oracle may only \emph{decrease} the value of~$\err_i[\qry]$.  

When a query $\UPO(i,\up)$ is made, the oracle first updates the data object~$\col_i$ and its corresponding representation.  
%
\ignore{The set~$\setC_i$ is reset to empty, because the data object~$\col_i$ is ``new'' again.
}
%
...
There are two immediate distinctions that can be made between different security games. First, we can ask whether it is possible for the adversary to predict the representations ahead of time, in which case the $\REPO$ and $\UPO$ oracles are largely unnecessary. Second, we can ask whether it is possible for the adversary, given its knowledge of the representations, to predict the outputs of $\QRYO$ ahead of time.

We use simulation-based definitions for these two notions. In particular, we say a data structure has independent representations up to some leakage $\textsf{lk}\colon\mathcal{D}\to\bits^*$ if there is a simulator consisting of two functions $\textsf{Sim}_d\colon\bits^*\to\mathcal{D}$ and $\textsf{Sim}_u\colon\mathcal{D}\times\bits^*\to\mathcal{U}$, such that $\textsf{Sim}_d(\textsf{lk}(\col))$ produces a set whose representation is indistinguishable from $\col$ and $\textsf{Sim}_u(\col',\textsf{lk}(\col))$ produces an update $\up'$ such that $\up'(\col')$ is indistinguishable from $\col$.
...
\begin{figure*}[t]
  \twoCols{0.25}
  {
     \raisebox{-5pt}{\experimentv{$\Exp{\mathrm{indqry}}_{\struct}(\advA)$}}\\[2pt]
      $\key \getsr \keys$\\
      $r \getsr \mathcal{R}$\\
      $\col \getsr \mathcal{D}$\\
      $\pub \getsr \Rep_K(\col)$\\
      $\qry \getsr \textsf{Sim}(\textsf{lk}(\pub),r)$\\
      if $\Qry_K(\pub,\qry) \neq r$ return 1\\
      $i \gets 0$\\
      $r' \getsr \advA^{\REPO(\cdot),\UPO(\cdot,\cdot)}(\pub,\qry)$\\
      return $[r = r']$
 }
 {
      \oraclev{$\REPO(\col)$}\\[2pt]
      $i \gets i + 1$\\
      $\pub_i \getsr \Rep_\key(\col)$\\
      return $\pub_i$
      \\[4pt]
