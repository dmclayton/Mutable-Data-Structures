...
One way to guarantee good security is to have a structure where we can demonstrate that the adversary, even given its knowledge of the representations of data structures, cannot predict the outputs of $\Qry$ on those structures ahead of time.

% Difference: in a counting filter, $\UPO$ can be used to determine the answer to queries; in a Bloom filter, this can still be done, but the update cannot be undone afterwards, and so the chance of finding a false positive using this method is 0
% can we capture the CBF version using just `each $\UPO$ call counts as a $\QRYO$'?
% no; adding elements, querying, removing all elements, adding different elements, and querying the same thing again provides information about the overlap between those two sets
% however, that requires 2(|set1|+|set2|) updates; could they do something similar just with queries?
% 'best case' for adversary: 
...
In an actual Bloom filter, even with salts and a secret key, it is not the case that 
