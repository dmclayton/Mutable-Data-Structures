...
%Because the adversary never queries an element which is already in the filter and never attempts to re-add an element to the filter, all queries and updates occur on arguments which are not elements of $\col$ and which either have not been previously sent to $\QRYO$ or which returned a value of 0 after being set to $\QRYO$. Of the two types of updates, the adversary will always prefer the latter, since this guarantees at least one new bit will be set in the filter, thereby slightly increasing the chance of future false positives.

%Now the game ignores the adversary's actual queries and instead makes $\QRYO$ and $\UPO$ calls for completely arbitrary elements. If a non-false-positive is found, the next $\UPO$ call adds that element to the set. Since this is optimal behavior, it does not reduce the adversary's advantage. At this point, an adversary's strategy can be viewed as a sequence of arbitrary $\QRYO$ and $\UPO$ calls, with nothing to distinguish any two queries of the same type from each other. Note that if a $\QRYO$ call produces a non-false-positive, the adversary will always next prefer an $\UPO$ call to a $\QRYO$ call, since further $\QRYO$ calls cannot alter the benefit of using $\UPO$ to add a non-false-positive, whereas using $\UPO$ to add the non-false-positive will increase the chances of finding false positives with further $\QRYO$ calls.

%Because $\UPO$ calls increase the benefit from $\QRYO$ calls, the only reason to not make all $\UPO$ calls first is the possibility that a call to $\QRYO$ will uncover a non-false-positive which slightly increases the effectiveness of the immediately following $\UPO$ call. At most $\min(q_T,q_U)$ calls to $\UPO$ can benefit from this. Assuming without loss of generality that the adversary halts upon finding a sufficient number of errors to win, it is therefore optimal to make all but $\min(q_T,q_U) + r$ calls to $\UPO$ before making any calls to $\QRYO$, since 

%But if $q_U > q_T$, some of these $\UPO$ calls cannot possibly benefit from a preceding $\QRYO$, meaning that the optimal strategy always involves making $q_U - q_T$ calls to $\UPO$ before making any calls to $\QRYO$. This produces a success rate identical to simply including $q_U - q_T$ additional elements in $\col$ and only using $q_T$ calls to $\UPO$ during the experiment. We therefore need only consider the case of $q_U \le q_T$.

%In~$\G_3$, any optimal adversarial strategy will have the property that its first $q_U$ calls to $\QRYO$ that do not return false positives will be immediately followed by an $\UPO$ query to add that element to the set. Assuming without loss of generality that the adversary halts upon finding a sufficient number of errors to win, these occur within the first $q_U + r$ calls to $\QRYO$. Finally we move to~$\G_4$, were the adversary no longer has direct access to $\UPO$. Instead we have modified $\UPO$ to no longer decrement $\err$ when a false positive is added to the set and $\QRYO$ to follow each of the first $r+q_U$ calls with an automatic $\UPO$ call.

%The adversary's advantage in~$\G_4$ is bounded abvove by the standard bound for a Bloom filter using $k$ hash functions and $m$ bits to store $n+r$ elements, where the adversary has $q_T$ attempts to find $r$ false positives. This gives

% up to $r+q_U$ calls to $\QRYO$ of the adversary's choice are followed by a free and "no-penalty" (no penalty for false positive, increase max size of set) $\UPO$ call, and all $\UPO$ calls are preceded by a free $\QRY$ call
% now have $q_R+q_U$ queries to a Bloom filter which never has more than $n+r$ elements; success chance is (q_T+q_U choose r)((1-e^(-k(n+r)/m))^k+O(1/n))^r
...
\begin{figure}
  ...
  }
  {
    ...
    \hspace*{-4pt}\rule{1.049\textwidth}{.4pt}
    \\[2pt]
    \oraclev{$\QRYO(i, \qry)$}\hfill\diffplus{$\G_4$}\;{$\G_3$}\hspace*{0pt}\\[2pt]
      $\ct' \gets \ct' + 1$\\
      \foreach{j}{1}{\ct}\\
      \tab $a_j \gets \Qryx[\Rnd](\pub_j, \qry)$\\
  ...
  \twoColsNoDivide{0.50}
  {
    \adversaryv{$\advB^{\PRFO}$} \comment{\prf adversary}\\[2pt]
      $\err_0, \ct \gets 0$;
      $\ky \getsr \keys$\\
      $i \getsr \advA^{\REPO',\QRYO'}$;
      return $(\err_i \geq r)$
  }
  {
    \oraclev{$\REPO'(\col)$}\\[2pt]
      ...
      $\col_\ct \gets \col$\\
      $\salt \getsr \bits^\lambda$;
      $\pub_\ct \gets \Repx[\PRFO](\col, \salt)$\\
      return $\pub_\ct$
    \\[6pt]
    \oraclev{$\QRYO'(i,\qry)$}\\[2pt]
      $\res \gets \Qryx[\PRFO](\pub_i, \qry)$\\
      if $\res \neq \qry(\col_i)$ then $\err_i\gets\err_i+1$\\
      return~$\res$
  }
  \caption{Games 0--4 and adversary~$\advA$ for the proof of
  Theorem~\ref{thm:bf-prf-correct}.}
...
Finally, in~$\G_4$ we keep track of how many $\QRYO$ calls have previously been made, and use $\setQ$ to keep track of those queries with indices in $\setI$. Only these $r$ queries can increment the error counters, but they do so if they cause an error in any of the currently-existing $\pub_j$. To win, the adversary must therefore return an $i$ so that all queries in $\setQ$ produce errors when directed at $\pub_i$. Assuming without loss of generality that the adversary does not add any elements to the set which it knows to be false positives, this condition is sufficient as well as necessary. Given that an adversary is successful in~$\G_3$, it is successful in~$\G_4$ with probability at least $\binom{q_T}{r}^{-1}$ since that is the chance of randomly selecting $r$ successful queries from the total collection of $q_T$ queries. Then we have $\Adv{\errep}_{\struct_\prfbloom,r}(\advA) \le \Adv{\prf}_F(\advB) + q_R^22^{-\lambda} + \binom{q_T}{r}\Prob{\G_4(\advA)=1}$.
...
%       key: same as public-rep case because construction of representations is deterministic
%     salt
%       no key: very messy bound (see above)
%       key: ?