...
Fix integers $q_R, q_T, q_H, q_V, r, t \geq 0$, let $p_\ell = ((\ell+1)/m)^k$ and
$r' = \lfloor r/\max(\delta^+,k\delta^-) \rfloor$. For all such $q_R, q_T, q_H,
r$, and~$t$, if $r' > p_\ell q_T$ then
  \begin{equation*}
  \begin{aligned}
